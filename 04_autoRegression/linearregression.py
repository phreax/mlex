...
class LinearModel:
    ...
    def __init__(self,getfeatures,x=None,y=None):
        self.x = x
        self.y = y
        self.train(getfeatures)

    def from_file(self,filename,getfeatures):
        self.x,self.y = loaddata(filename)
        self.train(getfeatures);

    def train(self,getfeatures):
        X = getfeatures(self.x)
        self.beta = leastsquarefit(X,self.y)
    ...
    def predict(self,getfeatures,dataX):
